...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i=1; i<height-1 ; i++)
    {
        ...
        {
            blurRed=(image[i-1][j-1].rgbtRed+image[i-1][j].rgbtRed+image[i-1][j+1].rgbtRed+image[i][j-1].rgbtRed+5*2*image[i][j].rgbtRed+image[i][j+1].rgbtRed+image[i+1][j-1].rgbtRed+image[i+1][j].rgbtRed+image[i+1][j+1].rgbtRed);
            blurGreen=(image[i-1][j-1].rgbtGreen+image[i-1][j].rgbtGreen+image[i-1][j+1].rgbtGreen+image[i][j-1].rgbtGreen+5*2*image[i][j].rgbtGreen+image[i][j+1].rgbtGreen+image[i+1][j-1].rgbtGreen+image[i+1][j].rgbtGreen+image[i+1][j+1].rgbtGreen);
            blurBlue=(image[i-1][j-1].rgbtBlue+image[i-1][j].rgbtBlue+image[i-1][j+1].rgbtBlue+image[i][j-1].rgbtBlue+2*5*image[i][j].rgbtBlue+image[i][j+1].rgbtBlue+image[i+1][j-1].rgbtBlue+image[i+1][j].rgbtBlue+image[i+1][j+1].rgbtBlue);
...
void edges(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i=1; i<height-1 ; i++)
    {
        for (int j=1 ; j<width-1 ; j++)
        {
            ...
            max(&edgeX[i][j].rgbtRed);
            max(&edgeX[i][j].rgbtGreen);
            max(&edgeX[i][j].rgbtBlue);
            max(&edgeY[i][j].rgbtRed);
            max(&edgeY[i][j].rgbtGreen);
            max(&edgeY[i][j].rgbtBlue);
